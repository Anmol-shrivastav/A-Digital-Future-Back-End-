...
if(isset($_POST['add']))   {
	...
	$sql = "INSERT INTO card(IMAGE,TITLE,DESCRIPTION) VALUES ('{$file_name}','{$title}','{$description}')";
    $result = mysqli_query($conn,$sql) or die("Query not running");
	$sql1 = "CREATE TABLE $title(ID INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, IMAGE VARCHAR(50) NOT NULL, TITLE VARCHAR(50) NOT NULL, FILENAME VARCHAR(50) NOT NULL)";
	$result1 = mysqli_query($conn,$sql1) or die("Create table query is not Running");
...
<div class="container-fluid">
 ...
 left" enctype="multipart/form-data">
   ...
   <p>Select Image *</p>
   <div class="form-group">
     <input type="file"  placeholder="Select Image" name="image" required>
   </div>
